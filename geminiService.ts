...
`.trim();
  ...
  try {
    const response = await fetch('http://localhost:5000/api/ai/agent', {
      ...
      body: JSON.stringify({
        prompt: fullPrompt,
        apiKey // Pass the key from frontend
      })
    });
...
export async function getStructuredWorkout(text: string): Promise<any> {
  ...
  try {
    console.log('Parsing workout from text:', text.substring(0, 200) + '...');
...
Return ONLY the raw JSON object, no markdown formatting.`,
    ...
    // Helper function to normalize exercises
    ...
    console.log('Successfully parsed workout:', safeParsed);
    return safeParsed;
  } catch (error) {
